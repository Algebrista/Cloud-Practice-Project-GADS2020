...
## Step1:
Sign into GCP (Google Cloud Platform) Console with the credentials provided by qwiklabs or using your account.
## Step 2:
Once you are inside GCP console, on the top right toolbar click the cloud shell icon to activate cloud shell. When prompted, click continue

## Task 1:
* Create a new virtual machine instance to list active account name, run the following command;

    --gcloud auth list

* To list the project ID, run the following command;

    --gcloud config list project qwiklabs-gcp-00-ead822e345d0
...
* To create a new virtual machine instance in the Cloud shell run the following command;
  ...
  --gcloud compute instances create -vm-1 --machine-type n1-standard-1 --zone us-central1-c --no-address

### (Note: choose any unique name for your instance)

This above command after running will give you an output like this;

Created [...augustine-matturi-vm-1].
NAME        ZONE           MACHINE_TYPE  ...    STATUS
augustine-matturi-vm-1  us-central1-c  n1-standard-1 ...    RUNNING
...
*  Use the compute instances create command to create a new instance and specify the image family for one of the Windows Server public images.
    ...
    gcloud compute instances create augustine-matturi-vm-2 \
...
*  Use the gcloud compute reset-windows-password command to create a new account and password or reset the existing account password for the logged in user:
   ...
   --gcloud compute reset-windows-password augustine-matturi-vm-2
...
[For more information, click:](https://cloud.google.com/compute/docs/operating-systems/windows#reset)
...
* To create a new  custom VM instance in the Cloud shell run the following command;
  ...
  --gcloud compute instances create augustine-matturi-vm-3 --custom-cpu 6 --custom-memory 32
...
To delete the custom VM above using the gcloud command-line tool, run the following command:
 ...
 gcloud compute instances delete augustine-matturi-vm-3 --zone=us-central2-a
